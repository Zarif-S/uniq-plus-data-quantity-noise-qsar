...
3. **Add FCNN + MPNN1/MPNN2 wrappers** — the real work (new sklearn-compatible estimator classes); base loop appends their rows incrementally.
   ...
   - ⏳ **MPNN1 + MPNN2** — next: one `ChempropRegressor(BaseEstimator, RegressorMixin)` over ChemProp's Python API, graph inputs (SMILES) threaded through the loop separately from `get_paper_models()` (not a tabular X-matrix model). Fetch ChemProp docs (context7) before writing.
