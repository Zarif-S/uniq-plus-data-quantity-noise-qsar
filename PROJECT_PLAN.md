...
3. **Add FCNN + MPNN1/MPNN2 wrappers** — the real work (new sklearn-compatible estimator classes); base loop appends their rows incrementally. NB: `param_base_FCNN` in `hyperparams.py:71` has a stray trailing comma making it a 1-tuple, not a dict — fix when wiring FCNN.
4. **Section 4.4 tuning pass** — heterogeneous tuners, decoupled.
...
- [ ] Section 5.7 — Table 2 Part 2 (CV Pearson r without parens, test Pearson r in parens, post-tuning)
