...
Four steps in order: Cleanup → FragmentParent → Uncharge → TautomerEnumerator.Canonicalize. Implemented in `src/preprocessing/preprocessing.py`. Causes small compound losses (−1 per endpoint for HLM/MDR1/SOL/RLM) due to `drop_duplicates('can_smi')` after canonical SMILES change.

**Correction (2026-08-19): this −1 loss is NOT something the paper's own script would also produce.** `ADME_ML_public.py` builds its row table as a `dict` keyed by `molName` (Vendor ID / `_Name` SD property) — `dtable[key] = tmpTable` — so it only collapses a row if two entries share the exact same compound ID. It never checks whether two *different* compound IDs standardize to the same canonical structure. Our loader explicitly does that check (`pd.DataFrame(rows).drop_duplicates('can_smi')` in `01.5_adme_biogen_public_recreation.ipynb`, cell `xiknojknoq`). So if the source data contains two distinct vendor entries that happen to standardize to the same molecule, their script keeps both as separate training rows (with — unless coincidentally identical — two different activity values for the "same" molecule); ours merges them into one. The compound-count match to the paper's published Table 2 (e.g. HLM 3086 vs. their 3087) is very likely coincidental, or partially offset by some other count difference, not "the same standardization loss occurring on both sides."
...
**FCNN — several of the paper's `MultitaskRegressor` kwargs never took effect in the paper's own code, confirmed against DeepChem 2.1.0 source (2026-08-06)**
`FCNN_public.py` (Computational-ADME repo) calls `dc.models.MultitaskRegressor(..., init="glorot_uniform", batchnorm=True, activation='relu', optimizer='adam', momentum=0.9, seed=5758)`. We initially treated `init`/`batchnorm`/`momentum` as things our port (`src/models/fcnn.py`, DeepChem 2.8.0, torch backend) should try to replicate. The paper was confirmed (by the project author) to have run on **DeepChem 2.1.0** — TensorFlow/TensorGraph-based, pre-PyTorch rewrite. Reading that exact version's source (`gh api repos/deepchem/deepchem/contents/...?ref=2.1.0`) shows `MultitaskRegressor.__init__(**kwargs)` → `TensorGraph.__init__(**kwargs)` → `Model.__init__(**kwargs)`, and `Model.__init__` never reads or stores its leftover `**kwargs` — any name that isn't an explicit parameter at every level in that chain is silently swallowed, no error raised.

For each kwarg: what the paper's script intended, what DeepChem 2.1.0 actually did with it, and what our port does —

- **`init="glorot_uniform"`** — intended: Glorot-uniform weight init. Actual: not a real parameter anywhere in the chain; every Dense layer is hardcoded to `weights_initializer=TFWrapper(tf.truncated_normal_initializer, stddev=weight_stddev)`. The paper's own reported results were produced with truncated-normal init, not Glorot, regardless of what the script says. **Ours**: `weight_init_stddevs=0.02` → truncated-normal(0.02) on every layer. This matches what DeepChem 2.1.0 actually ran, not because we deliberately reproduced their bug, but because that's simply how our `FCNN` class was already built (`weight_init_stddevs` is a real, working parameter in DeepChem 2.8.0). We are not claiming to have "corrected" or "faithfully reproduced" anything here — it's a coincidental match between our design and their code's actual behavior, worth recording so nobody later "fixes" `weight_init_stddevs` to try to add Glorot and unknowingly diverges from what the paper's numbers actually came from.
- **`batchnorm=True`** — intended: batch normalization between layers. Actual: not a real parameter; no `BatchNorm` layer exists anywhere in `fcnet.py` at 2.1.0. The paper's reported FCNN results never had batch norm, despite the script implying otherwise. **Ours**: also no batch norm (DeepChem 2.8.0's torch `MultitaskRegressor` dropped support for it entirely — see `src/models/fcnn.py`'s docstring). Same absence, but for a different and unrelated reason (their API silently discarding an unrecognized kwarg vs. our API never having implemented the feature at all). Not a case of "we matched them" — two independent failures to deliver the same intended feature.
- **`optimizer='adam'`, `momentum=0.9`** — intended: Adam with momentum/beta1 0.9. Actual: neither kwarg is a real parameter; `TensorGraph.__init__` hardcodes `self.optimizer = Adam(learning_rate=learning_rate, beta1=0.9, beta2=0.999, epsilon=1e-7)` regardless of what's passed. The paper's actual beta1 was always 0.9, by hardcoding, not by the `momentum=0.9` they wrote. **Ours**: `beta1=0.9` (`param_base_FCNN`), a real, functioning parameter in DeepChem 2.8.0's `dc.models.optimizers.Adam`. Net numerical effect matches (0.9 either way) — again coincidental, not a deliberate correction, since a paper script written with `momentum=0.5` would have looked different but still silently produced beta1=0.9 with no way to detect the mismatch from the script alone.
- **`activation='relu'`** — intended: ReLU. Actual: wrong parameter name (`activation_fns` is the real one), so the kwarg is discarded — but `activation_fns` already defaults to `tf.nn.relu`, so the paper's actual result is ReLU regardless. **Ours**: `activation_fns='relu'`, explicit and correctly named. The only kwarg here where intent, the paper's actual behavior, and our behavior all agree with no caveats.
- **`seed=5758`** — intended: reproducible runs. Actual: wrong parameter name (`random_seed` is the real one); `tf.set_random_seed()` is only called when `random_seed` is set, so this never fired. The paper's FCNN runs were almost certainly not seeded, despite the script's apparent intent. **Ours**: `FCNN.fit()` calls `np.random.seed(self.random_state)` and `torch.manual_seed(self.random_state)` directly — real, global calls, not routed through any DeepChem kwarg that could be silently dropped. This is the one place we deliberately diverge from what the paper's code actually did, and it's a clear improvement: our runs are genuinely seeded, theirs (as run) were not.

**Bottom line**: no code changes made as a result of this — `param_base_FCNN` already happens to reproduce what DeepChem 2.1.0 actually did for weight init and Adam's beta1 (coincidence of how the class was built, not a deliberate reproduction effort), batch norm is absent on both sides for unrelated reasons, and seeding is a deliberate, genuine improvement over the paper's actual (non-reproducible) behavior. Recorded here so this distinction — matched-by-coincidence vs. deliberately-corrected — doesn't get flattened into "we replicated the paper's FCNN faithfully" in the writeup.
...
### Correction (post-hoc, same day)

The original "Rationale" below claimed the affected descriptors "never survived to the final
5-descriptor selection anyway." **That's wrong.** `data/processed/selected_descriptors.json`
shows `CalcEccentricity` and `CalcPMI3` — 2 of the 9 flat-geometry descriptors — *are* in the
final 5-descriptor set, alongside `CalcTPSA`, `PEOE_VSA`, `SlogP_VSA`. That's 40% of the selected
descriptors, not zero. See the *Why they're still predictive* subsection under Context for the
follow-up analysis, and the *Future work* section for the low-cost diagnostic this motivates.
The flat-geometry finding itself (all `z=0`, `Is3D()=False`) was independently re-confirmed at
full dataset scale (see below) and stands as originally reported — only the "limited impact"
claim was wrong.
...
While auditing the rdmoldes descriptor set during feature selection (`01.8_feature_selection.ipynb`),
several "3D shape" descriptors showed anomalous behaviour on the ADME dataset:

- `CalcPBF` and `CalcSpherocityIndex` returned **exactly 0.0** for all 3509 unique molecules
  (not just near-zero — literally identical).
- `CalcNPR1 + CalcNPR2 = 1.0` **exactly** for every molecule (Pearson r = −1.0).

Both are the signature of a perfectly *planar* (flat, 2D) rigid body: a best-fit plane trivially
contains every atom when they're already coplanar (`CalcPBF=0`), RDKit's spherocity formula
reduces to 0 for planar geometry, and `NPR1+NPR2=1` follows exactly from the perpendicular axis
theorem for any flat object.

Checked directly against the raw SDF files (`data/sdfs/ADME_*.sdf`): every atom's `z` coordinate
is `0.0` and `conformer.Is3D()` returns `False`. The SDFs only contain 2D depiction coordinates —
the layout used for drawing the structure — not embedded 3D conformers. Re-confirmed at full
scale (not just the 3509 unique molecules): every molecule across all 6 SDF files (13,649 total,
including duplicates across endpoints) has `max|z|=0.0` and `Is3D()=False`, no exceptions.

This affects 9 of the 50 rdmoldes descriptors: `CalcPBF`, `CalcSpherocityIndex`, `CalcNPR1`,
`CalcNPR2`, `CalcPMI1-3`, `CalcAsphericity`, `CalcEccentricity`, `CalcRadiusOfGyration`,
`CalcInertialShapeFactor`. Two (`CalcPBF`, `CalcSpherocityIndex`) are entirely uninformative as a
result (correctly dropped by `01.8`'s constant-variance filter). The rest still show real
variation, but reflect 2D-layout shape (elongation, branching in the drawing plane), not genuine
3D molecular shape.

**This is inherited from the original paper, not introduced by this project.** The reference
script (`ADME_ML_public.py`, Fang et al. 2023 / Computational-ADME) loads mols directly via
`Chem.SDMolSupplier(...)` and computes the same 3D descriptors immediately afterward — no
`EmbedMolecule`/`MMFFOptimizeMolecule` step anywhere in their pipeline either. The paper's own
published `rdMolDes` results carry the same limitation.

#### Why `CalcEccentricity` and `CalcPMI3` are still predictive despite being flat

Their predictive power has two different explanations, which matters for how much to trust each:

- **`CalcPMI3`** correlates moderately-strongly with `CalcExactMolWt` (r=0.58), heavy-atom count
  (r=0.58), and rotatable-bond count (r=0.54). A good chunk of its signal is likely just molecular
  size in disguise — even on a flat 2D layout, a bigger molecule occupies more area, so its moment
  of inertia grows with size, which is a well-known real driver of these ADME endpoints. Notably,
  `CalcExactMolWt` itself was pruned earlier by `correlation_prune` (dropped in favor of
  `AUTOCORR2D`) — so `CalcPMI3` may partly be surviving downstream as an indirect stand-in for the
  size signal `CalcExactMolWt` would otherwise have carried.
- **`CalcEccentricity`** is a different story — only weakly correlated with molecular weight,
  heavy-atom count, ring count, or rotatable bonds (all `|r| < 0.11`). Its predictive value isn't
  just size in disguise; it's capturing something more independent, plausibly the
  branching/elongation pattern of how RDKit laid the structure out in 2D. **This is a reason for
  more skepticism, not less**: 2D layout coordinates are generated by RDKit's drawing heuristics,
  not a physically canonical quantity — the same molecule could get a different layout from a
  different tool or RDKit version. `CalcEccentricity` being predictive here doesn't mean it's
  measuring something structurally principled; it may be encoding an artifact of RDKit's specific
  2D-layout algorithm applied to this dataset's structures, which might not transfer or reproduce
  if the coordinates were regenerated differently.

---

### Decision

Leave `rdmoldes()` and the `01.5` featurization pipeline as-is (no conformer embedding added).
Document the limitation here rather than fix it silently.

---

### Rationale

- **Faithfulness to the paper.** `01.5`'s stated purpose is reproducing Fang et al.'s methodology.
  Adding real 3D conformer generation would deliberately diverge from what the paper did,
  undermining the reproduction rather than improving it.
- **Scope.** Fixing this properly (`AllChem.EmbedMolecule` + force-field optimization, choice of
  method/seed/conformer count) is a nontrivial upstream change that would require re-running
  every downstream artifact touching the `rdkit`/`hybrid` featuresets or MPNN3's hybrid input —
  out of scope for the feature-selection work that surfaced it.
- ~~**Limited practical impact so far.**~~ **Corrected — see above.** `01.8`'s constant-filter
  does remove the two fully degenerate descriptors on data alone, but 2 of the remaining 7
  (`CalcEccentricity`, `CalcPMI3`) *are* in the final 5-descriptor selection — 40% of it. This
  finding is directly relevant to the current feature-selection result, not just a latent risk in
  unused descriptors.
...
Any "3D shape" descriptor from `rdmoldes()` used anywhere in this project (`01.5`/`01.6`'s
`rdkit`/`hybrid` featuresets, MPNN3's hybrid input) should be understood as encoding 2D-layout
shape, not true 3D conformational geometry. This directly affects the `01.8` feature-selection
result: `CalcEccentricity` and `CalcPMI3`, 2 of the 5 selected descriptors, are computed on flat
coordinates. `X_rdkit`, `X_hybrid`, and `X_hybrid_ecfp4` in `01.5` (built via `rdmoldes(mols)` on
mols loaded straight from the flat SDFs) are exposed to this; `rdkit_norm`/MPNN2's descriptors
come from a separate `descriptastorus` function and are unaffected. Worth a caveat in the writeup
if 3D-descriptor importance/performance is discussed, and worth resolving with the diagnostic
below before treating the current 5-descriptor selection as final.

---

### Step 1 diagnostic — done, result

**Ran 2026-08-10** in `01.8_feature_selection.ipynb` §7. First pass used one `AllChem.EmbedMolecule`
(ETKDGv3) + `MMFFOptimizeMolecule` per molecule (seed=42, retried on embedding failure only) — on
review this was flagged as a real methodological gap: several of the affected descriptors
(`CalcAsphericity`, `CalcEccentricity`, `CalcPMI1-3`, `CalcRadiusOfGyration`,
`CalcInertialShapeFactor`) can vary across a flexible molecule's accessible conformers, so one
arbitrarily-seeded embedding is only a single snapshot, not a representative "real 3D geometry."
**Revised to a 5-conformer ensemble per molecule (ETKDGv3), descriptors computed on each
molecule's lowest-MMFF-energy conformer** — the standard cheap proxy for the geometry a molecule
is actually most likely to occupy. **0 embedding failures across all 3509 unique molecules.** The
9 affected descriptors that survive the constant-filter were recomputed on those lowest-energy
conformers and spliced into `01.8`'s existing per-endpoint feature matrices in place of the flat
values (`CalcPBF`/`CalcSpherocityIndex` stay excluded — already constant-filtered, not
reconsidered). Full result: `data/processed/adr011_3d_diagnostic.json`; per-endpoint breakdown in
`data/processed/adr011_3d_diagnostic_comparison.csv`.

| Descriptor set | Flat CV R² (mean) | Real-conformer CV R² (mean, lowest-energy of 5) | Δ |
|---|---|---|---|
| Final 5-descriptor selection (`CalcTPSA`, `CalcEccentricity`, `CalcPMI3`, `PEOE_VSA`, `SlogP_VSA`) | 0.3555 | 0.3638 | **+0.0083** |
| 48-descriptor post-constant-filter baseline | 0.4193 | 0.4325 | **+0.0131** |

Real conformers give a small, **positive** delta at both stages — real geometry never made things
worse, and the ensemble/lowest-energy revision pushed both deltas up slightly from the original
single-conformer pass (+0.0064→+0.0083, +0.0101→+0.0131), consistent with a single arbitrary
conformer having somewhat *understated* the real effect. Per-endpoint, SOL benefits most from real
geometry (+0.025 on the 5-descriptor set, +0.040 on the 48-descriptor baseline) — plausible, since
aqueous solubility is more directly shape/packing-sensitive than the other 3 endpoints.

Both deltas remain smaller in magnitude than the R² cost of every pruning stage in the pipeline
(correlation-prune: −0.0137, VIF: −0.0233, RFE: −0.0269) — the 48-descriptor delta (+0.0131) is
now close to correlation-prune's cost (−0.0137) but still under it. The flat-vs-real geometry
choice matters less to this selection's actual predictive performance than any one of the pruning
decisions already being made. This is a soft negative result: the 2D-layout proxy was already
capturing most of what these descriptors have to offer for this dataset, plausible per ADR-011's
original size-driven-conformational-restriction hypothesis for small drug-like molecules — not a
strong enough signal to prioritize Step 2 below, but not a reason to fully dismiss the
descriptors' real-3D upside either, particularly for SOL specifically.

### Future work

**Step 2 — full propagation (not currently prioritized):** the Step 1 delta above is real but
small — smaller than any single pruning stage's own R² cost — so this is not currently justified
as a priority, but remains available if 3D-descriptor performance becomes more central to the
writeup: explicit conformer embedding added to `01.5`'s pipeline as a separate branch/arm (not
replacing the paper-faithful featureset), retraining RF/XGBoost/LightGBM/Ridge/BayesianRidge/FCNN
across `rdkit`/`hybrid`/`hybrid_ecfp4` × the 4 modelling endpoints, and re-diffing against
`results_df`.
